...
2. **정원/현원** — ⚠️ *ALIO 항목번호 확정 필요* (아래 Open Question)
...
- [ ] **A-1. 기준점(ground truth) 기간 정합** — 현재 HTML=2026 Q1 vs 검증 rawdata=2025 Q1 불일치 해소. 2026 Q1 공식 XLSX 확보 또는 대표 기관 재크롤. (spec §5-4, transition_plan "Why Match Rate Was Not 100%")
...
- [ ] **A-3. 항목·값 단위 대조 하네스** — v2 → ALIO 원문 값 비교, **항목별 정확 일치율** 리포트. 불일치를 5분류: `기간차 / 미크롤 / 첨부전용 / 파서구조손실 / 정정공시`. (기존 `build_metrics_from_canonical.py`의 v1비교 로직을 골든/원문 대조로 승격)
- [ ] **A-4. 라이브 ALIO 대조를 v2에 연결** — `validate_live_alio.py`가 v2 canonical도 검사하도록 확장(분기 1회). (spec §5-4, §7.2)
...
## 4. Open Questions (사용자 확인 필요)

- [ ] **정원/현원의 정확한 ALIO 항목번호** — 어떤 item_no인지 확정해야 C-2 착수 가능.
- [ ] **2026 Q1 기준점 확보 가능 여부** — 기간 맞춘 공식 XLSX/원본을 구할 수 있는가(A-1 전제).
